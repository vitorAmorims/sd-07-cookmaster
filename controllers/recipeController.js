...
recipeRoute.put('/:id/image', authMiddleware, upload.single('image'), async (req, res, next) => {
  const { id } = req.params;
  const { user } = req;
  const image = req.file;
  const newPatch = `localhost:3000/images/${image.filename}`;
  try {
    const result = await recipeService.insertImageRecipeByIdService(id, user, newPatch);
    if (result.isError) return next(result);
    return res.status(status.SUCCESS).json(result);
  } catch (err) {
    throw new Error(err);
